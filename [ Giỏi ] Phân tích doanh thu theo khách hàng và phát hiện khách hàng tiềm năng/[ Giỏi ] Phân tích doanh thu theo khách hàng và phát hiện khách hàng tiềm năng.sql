...
SELECT c.customer_name, SUM(total_price) AS total_revenue, COUNT(*) AS order_count
...
HAVING SUM(total_price) > (
	...
	FROM (
		SELECT c.customer_id, SUM(total_price) AS total_revenue
		FROM customers c JOIN orders o ON c.customer_id = o.customer_id
...
SELECT c.customer_name, c.city, COUNT(*) AS products_count, SUM(i.price * i.quantity) 
FROM customers c 
